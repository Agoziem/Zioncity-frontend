...
const ClassResultDatatableitems = ({currentItems, loading, setItems}) => {
    const SubjectResults = currentItems && currentItems.length > 0 ? currentItems[0].subjects_total : [];
    ...
    {loading ? (
        <tr>
            <td colSpan='17' className='text-center p-2'>Loading...</td>
        </tr>
    ) : (
        <>
            ...
            <tr key={item.id} item={item}>
                ...
                <>
                {
                    item.subjects_total.map((subject, index) => (
                        <td key={index} className='mx-7 p-2'>{subject.subject_total}</td>
                    ))
                }
                </>
                ...
                <td className={`mx-7 p-2 ${getColorClass(item.Remark)} fw-bold`}>{item.Remark}</td>
            </tr>
        ))) : (
                <tr>
                    <td colSpan='17' className='text-center p-2'>No Result Found for your Students </td>
                </tr>
            )}
